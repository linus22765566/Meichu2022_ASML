...
def get_all():
    ...
    for i in a:
        ...
        if(flag and flag2):
            ...
            d['max'] = i[6]
            d['comment'] = i[7]
            d['url'] = "https://3778-140-113-124-40.ngrok.io/detail/"+str(i[0])
            all.append(d)
    j = json.dumps(all)
    return j
...
def get_Detail():
    ...
    for i in a :
        ...
        d['max'] = i[6]
        d['comment'] = i[7]
        d['url'] = "https://3778-140-113-124-40.ngrok.io/detail/"+str(id)
        all.append(d)
    j = json.dumps(all)
    return j
...
def add_Form():
    ...
    url = "https://3778-140-113-124-40.ngrok.io/detail/"
    connection = sqlite3.connect('test.db')
    cursor = connection.cursor()
    ...
    get_id = get_id[0][0]
    return_url = "https://3778-140-113-124-40.ngrok.io/detail/" + str(get_id)
    connection.commit()
    res = {}
    if(get_id):
        res['url'] = return_url
...
def reserve():
    ...
    print(left_count)
    if left_count<0:
        left_count=left_count + int(amount)
        connection.commit()
        return "405 數量不足"
    else:
